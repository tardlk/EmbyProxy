    ...
    public static class HandlerInterceptor
    {
        ...
        public static void Apply()
        {
            ...
            try
            {
                ...
                if (_factoryField == null) return;

                _originalFactory = _factoryField.GetValue(_httpClientManager) as Delegate;
                if (_originalFactory == null) return;

                RefreshCache();

                var wrapper = CreateWrapper(_originalFactory);
                _factoryField.SetValue(_httpClientManager, wrapper);
                Plugin.Instance.Logger.Info("HandlerInterceptor - factory wrapped");

                if (Plugin.Instance.DebugMode)
                {
                    if (_ipv4Enabled && _ipv4Domains.Length > 0)
                        Plugin.Instance.Logger.Info($"IPv4 only enabled for: {string.Join(", ", _ipv4Domains)}");
                    if (_tmdbRewrite)
                    {
                        if (!string.IsNullOrEmpty(_tmdbApiReplacement))
                            Plugin.Instance.Logger.Info($"TMDB API rewrite: → {_tmdbApiReplacement}");
                        if (!string.IsNullOrEmpty(_tmdbImageReplacement))
                            Plugin.Instance.Logger.Info($"TMDB Image rewrite: → {_tmdbImageReplacement}");
                    }
                }
            }
            catch (Exception e)
            {
                Plugin.Instance.Logger.Warn($"HandlerInterceptor failed: {e.Message}");
            }
        }

        public static void RefreshCache()
        {
            var opts = Plugin.Instance.MainOptionsStore.GetOptions();

            _ipv4Enabled = opts.NetworkOptions.EnableIPv4Only;
            _ipv4Domains = opts.NetworkOptions.IPv4OnlyDomains
                ?.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(d => d.Trim())
                .Where(d => d.Length > 0)
                .ToArray() ?? Array.Empty<string>();

            _tmdbRewrite = opts.TmdbOptions.EnableAltTmdb;
            _tmdbApiReplacement = opts.TmdbOptions.AltTmdbApiUrl?.Trim().TrimEnd('/');
            _tmdbImageReplacement = opts.TmdbOptions.AltTmdbImageUrl?.Trim().TrimEnd('/');
        }
        ...
        public static void Remove()
        {
            ...
            try
            {
                ...
                _applyCount = 0;
                Plugin.Instance.Logger.Info("HandlerInterceptor - factory restored");
            }
            catch (Exception e)
            {
                Plugin.Instance.Logger.Warn($"HandlerInterceptor remove failed: {e.Message}");
            }
        }
                ...
                sh.ConnectCallback = async (context, cancellationToken) =>
                {
                    try
                    {
                        ...
                        foreach (var d in domains)
                        {
                            ...
                            if (debug)
                                Plugin.Instance.Logger.Info($"IPv4 forced: {host} → {addresses[0]}");
                ...
                HttpRequestMessage request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(_tmdbApiReplacement) && url.Contains(TmdbApiHost))
                    {
                        ...
                        if (debug)
                            Plugin.Instance.Logger.Info($"TMDB API rewrite: {url} → {newUrl}");
                    ...
                    if (!string.IsNullOrEmpty(_tmdbImageReplacement) && url.Contains(TmdbImageHost))
                    {
                        ...
                        if (debug)
                            Plugin.Instance.Logger.Info($"TMDB Image rewrite: {url} → {newUrl}");
                        request.RequestUri = new Uri(newUrl);
                    }
                }
